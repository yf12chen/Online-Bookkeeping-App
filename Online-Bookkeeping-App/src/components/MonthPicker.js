...
class MonthPicker extends React.Component {
  ...
  handleClick = e => {
    ...
    this.props.monthYearSelected(this.state.selectedYear, this.state.selectedMonth)
  };
  toggleDropDown = e => {
    e.preventDefault();
    this.setState({
            ...
            <div className="row">
              <div className="col border-right years-range">
                {yearRange.map((i, index) => (
                  <a role="button" className={`${i === selectedYear ? "dropdown-item active" : "dropdown-item"}`}
                    href="/#"
                    key={index}
                    onClick={e => {
                      this.selectedYear(e, i);
                    }}
                  >
                    {i}
                  </a>
